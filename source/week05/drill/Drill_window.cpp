...
Drill_window::Drill_window(Point xy, int w, int h, const string& title)
    :Window{xy,w,h,title},
    //we are creating the buttons here
    next_button{Point{x_max()-300,0}, 150, 40, "Next point",
        [](Address, Address pw) {reference_to<Drill_window>(pw).next();}},
    quit_button{Point{x_max()-150,0}, 150, 40, "Quit",
        [](Address, Address pw) {reference_to<Drill_window>(pw).quit();}},
    ...
    color_menu_button{Point{x_max()-300, 40}, 150, 40, "Colors",
    	[](Address, Address pw) {reference_to<Drill_window>(pw).toggle_color_menu();}},
    color_menu{Point{x_max()-300, 40}, 150, 40, Menu::vertical, "color"},

    line_style_menu_button{Point{x_max()-150, 40}, 150, 40, "Line Styles",
    	[](Address, Address pw) {reference_to<Drill_window>(pw).toggle_line_style_menu();}},
    line_style_menu{Point{x_max()-150, 40}, 150, 40, Menu::vertical, "lines"}
{
    ...
    [](Address, Address pw) {reference_to<Drill_window>(pw).change_color(Color::red);} });  
      
    color_menu.attach(new Button{Point{0, 0}, 0,0, "Blue", 
    [](Address, Address pw) {reference_to<Drill_window>(pw).change_color(Color::blue);} }); 
    
    color_menu.attach(new Button{Point{0, 0}, 0,0, "Black", 
    [](Address, Address pw) {reference_to<Drill_window>(pw).change_color(Color::black);} }); 

    color_menu.attach(new Button{Point{0, 0}, 0,0, "Green", 
    [](Address, Address pw) {reference_to<Drill_window>(pw).change_color(Color::green);} }); 

    attach(color_menu);    
    color_menu.hide();  

    line_style_menu.attach(new Button{Point{0, 0}, 0,0, "Solid", 
    [](Address, Address pw) {reference_to<Drill_window>(pw).change_line_style(Line_style::solid);} });  
      
    line_style_menu.attach(new Button{Point{0, 0}, 0,0, "Dashed", 
    [](Address, Address pw) {reference_to<Drill_window>(pw).change_line_style(Line_style::dash);} });  
      
    line_style_menu.attach(new Button{Point{0, 0}, 0,0, "Dotted", 
    [](Address, Address pw) {reference_to<Drill_window>(pw).change_line_style(Line_style::dot);} });  
 
    attach(line_style_menu);
    line_style_menu.hide();
}

void Drill_window::change_color(Color color) { 
	lines.set_color(color); 
	toggle_color_menu();
}

void Drill_window::change_line_style(Line_style style){
	lines.set_style(style);
	toggle_line_style_menu();
}

void Drill_window::toggle_color_menu(){
    if(isColorMenuShown){
        color_menu.hide();
...
void Drill_window::toggle_line_style_menu(){
...
void Drill_window::quit()
{
    hide();
}

void Drill_window::next()
{
