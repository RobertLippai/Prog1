/*
	g++ vector_007.cpp -o vector_007.out -std=c++14 && ./vector_007.out
*/
...
public:
	...
	using size_type = unsigned long; //type alias
	using value_type = T;
	using iterator = T*;
	...
	//utolsó utáni elemre mutat az end
	...
	//allocate reserves s memory space for Type T
	explicit My_vector(int s): sz(s), elem{alloc.allocate(s)}, space(s) 
	{
		for (int i = 0; i < s; ++i) alloc.construct(&elem[i], T()); //this creats the objects
	...
	{
		for (int i = 0; i < sz; ++i) alloc.construct(&elem[i], *(lst.begin()+i)); //dereferenc the iterator pointer
		//copy(lst.begin(), lst.end(), elem);
		//debug("lst constructor");
	}

	My_vector(const My_vector& arg): 
		sz{arg.sz}, elem{alloc.allocate(sz)}, space{static_cast<int>(arg.sz)}
	{
		for (int i = 0; i < sz; ++i) alloc.construct(&elem[i], arg[i]); //const reference
	...
	My_vector& operator=(const My_vector& arg)
	{
		...
		for (int i = 0; i < arg.sz; ++i) alloc.construct(&p[i], arg[i]); //dereferenc the iterator pointer
	...
	void reserve(int newalloc)
	{
		...
		for (int i = 0; i < sz; ++i) alloc.destroy(&elem[i]); // destroys object
		alloc.deallocate(elem, space); // frees memory
		//delete[] elem;
		elem = p;
		space = newalloc;
	}
	...
	void resize(int newsize, T val = T())
	{
		...
		for (int i = sz; i < newsize; ++i) alloc.construct(&elem[i], val); // if we increase size
		for (int i = newsize; i < sz; ++i) alloc.destroy(&elem[i]); // if we decrease size
	...
	iterator insert(iterator p, const T& val){
		int index = p-begin(); // how many space between this element and the start
		if(size() == capacity()){
			//if the vector is empty we will reserve 8 spaces
...
try {
	...
	*/
	...
	My_vector<string> swm { "Liza", "MDNA", "Rose", "Rikko", "Jen", "Pat", "Jill", "Jhon"};
	...
	auto p = find(swm.begin(), swm.end(), "Liza");

	auto pj = find(swm.begin(), swm.end(), "Jill");

	swm.erease(pj);
	swm.insert(pj, "Joseph");

	*p = "Bill";

	cout << endl;

	for (const auto& e: swm)
	{
		cout << e << endl;
	}
